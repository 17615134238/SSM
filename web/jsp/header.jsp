<%--
  Created by IntelliJ IDEA.
  User: Administrator
  Date: 2018/4/21 0021
  Time: 8:33
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>Title</title>
    <link rel="stylesheet" href="/SSM/css/bootstrap.min.css">
    <link href="/SSM/css/header.css" rel="stylesheet">
</head>
<body id="all" onload="riqi()">
<header class="container-fluid">
    <div class="row">
        <div class="col-md-5 col-xs-4">
            <a href="shouye.jsp">
                <img src="/SSM/image/logo.png">
            </a>
        </div>
        <div class="col-md-3 col-xs-4">
            <span class="wenben">商家管理系统</span>
        </div>
        <div class="col-md-2 col-md-offset-2 col-xs-4 right">
            <span id="time" class="t"></span>
        </div>
    </div>
</header>
<script src="/SSM/js/jquery1.12.4.min.js"></script>
<script src="/SSM/js/bootstrap.min.js"></script>
<script src="/SSM/js/header.js"></script>
</body>
</html>
